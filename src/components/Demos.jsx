...
const Demos = () => {
  ...
  const openVideo = (videoSrc) => setActiveVideo(videoSrc);
  ...
  return (
    <section id="demos" className="py-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card>
          <div className="relative z-10 flex flex-col h-full">
            <div 
              className="w-full aspect-video bg-black/50 rounded-xl mb-6 border border-white/[0.08] flex gap-2 items-center justify-center text-[#86868b] text-sm cursor-pointer hover:bg-white/5 transition-all group"
              onClick={() => openVideo(sakadoVideo)}
            >
              <svg className="w-8 h-8 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" fillRule="evenodd"></path></svg>
              <span className="group-hover:text-white transition-colors">Lire la vidéo</span>
            </div>
        ...
        <Card>
          <div className="relative z-10 flex flex-col h-full">
            <div 
              className="w-full aspect-video bg-black/50 rounded-xl mb-6 border border-white/[0.08] flex gap-2 items-center justify-center text-[#86868b] text-sm cursor-pointer hover:bg-white/5 transition-all group"
              onClick={() => openVideo(somebuddyVideo)}
            >
              <svg className="w-8 h-8 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" fillRule="evenodd"></path></svg>
              <span className="group-hover:text-white transition-colors">Lire la vidéo</span>
            </div>
        ...
        <Card>
          <div className="relative z-10 flex flex-col h-full">
            <div 
              className="w-full aspect-video bg-black/50 rounded-xl mb-6 border border-white/[0.08] flex gap-2 items-center justify-center text-[#86868b]/50 text-sm cursor-not-allowed"
              title="Vidéo non disponible pour le moment"
            >
              <svg className="w-8 h-8 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg>
      ...
      {activeVideo && (
        ...
        >
          <div 
            className="relative w-full max-w-5xl bg-black rounded-xl overflow-hidden border border-white/10 shadow-2xl" 
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              className="absolute top-4 right-4 z-[60] bg-black/60 text-white rounded-full w-10 h-10 flex items-center justify-center hover:bg-white/20 transition-colors"
              onClick={closeVideo}
